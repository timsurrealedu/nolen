...
role() {
  password="$(cat "/run/nolen-secrets/postgres_${1}_password")"
...
GRANT SELECT, UPDATE ON incidents TO nolen_api;
GRANT SELECT, INSERT ON incident_status_audit TO nolen_api;
GRANT USAGE, SELECT ON SEQUENCE incident_status_audit_audit_id_seq TO nolen_api;
SQL
