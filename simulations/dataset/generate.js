...
export function generateDataset({ seed = 20260721, hostCount = 24, normalSessionsPerHost = 30, attackRuns = 24, start = '2026-01-01T00:00:00.000Z' } = {}) {
  ...
  for (let index = 1; index <= hostCount; index += 1) {
    for (let session = 0; session < normalSessionsPerHost; session += 1) {
      ...
      add({ scenario: 'normal_operations', stage: 'normal_session', label: 'normal', isMalicious: false, body: { timestamp: iso(timestamp + 2_000), host: machine, user: { name: identity }, event: { category: 'process', action: 'start' }, process: { name: pick(processNames, next), pid: 1_000 + sequence, privilege: 'standard' } } });
      if (session % 4 === 0) add({ scenario: 'normal_operations', stage: 'routine_file_access', label: 'normal', isMalicious: false, body: { timestamp: iso(timestamp + 4_000), host: machine, user: { name: identity }, event: { category: 'file', action: 'access' }, file: { path: '/etc/passwd' } } });
    }
  }
  ...
  for (let run = 0; run < attackRuns; run += 1) {
    ...
    add({ scenario: 'authorized_maintenance', stage: 'privileged_activity', label: 'authorized_maintenance', isMalicious: false, body: { timestamp: iso(timestamp + 3_000), host: machine, user: { name: identity }, event: { category: 'process', action: 'start' }, process: { name: 'bash', pid: 8_000 + sequence, privilege: 'elevated' } } });
    add({ scenario: 'authorized_maintenance', stage: 'file_administration', label: 'authorized_maintenance', isMalicious: false, body: { timestamp: iso(timestamp + 6_000), host: machine, user: { name: identity }, event: { category: 'file', action: 'modify' }, file: { path: run % 2 ? '/etc/sudoers' : '/etc/cron.d/maintenance' } } });
  }
  ...
  for (let run = 0; run < attackRuns; run += 1) {
    ...
    add({ scenario: 'ssh_compromise', stage: 'privilege_escalation', label: 'privileged_activity', isMalicious: true, body: { timestamp: iso(timestamp + 31_000), host: machine, user: { name: identity }, event: { category: 'process', action: 'start' }, process: { name: 'bash', pid: 12_000 + sequence, privilege: 'elevated' } } });
    add({ scenario: 'ssh_compromise', stage: 'credential_access', label: 'sensitive_file_access', isMalicious: true, body: { timestamp: iso(timestamp + 36_000), host: machine, user: { name: identity }, event: { category: 'file', action: 'access' }, file: { path: '/etc/shadow' } } });
  }
